        ...
        @can('delete', $outlet)
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <p>{{ $outlet->types }}</p>
                    <label class="control-label text-primary">{{ __('outlet.wilaya') }}</label>
                    <p>{{ $outlet->numWilaya }}</p>
        ...
        <div class="card">
            ...
            <form method="POST" action="{{ route('outlets.update', $outlet) }}" accept-charset="UTF-8">
                ...
                <div class="card-body">
                    ...
                    <div class="form-group">
                        <label for="types" class="control-label">{{ __('outlet.type') }}</label>
                        <input id="types" type="text" class="form-control{{ $errors->has('types') ? ' is-invalid' : '' }}" name="types" value="{{ old('types', $outlet->types) }}" required>
                        {!! $errors->first('types', '<span class="invalid-feedback" role="alert">:message</span>') !!}
                    </div>
                    <div class="form-group">
                        <label for="numWilaya" class="control-label">{{ __('outlet.wilaya') }}</label>
                        <input id="numWilaya" type="number" class="form-control{{ $errors->has('numWilaya') ? ' is-invalid' : '' }}" name="numWilaya" value="{{ old('numWilaya', $outlet->numWilaya) }}" required>
                        {!! $errors->first('numWilaya', '<span class="invalid-feedback" role="alert">:message</span>') !!}
                    </div>
                    <div class="form-group">
                        <label for="address" class="control-label">{{ __('outlet.address') }}</label>
                        <textarea id="address" class="form-control{{ $errors->has('address') ? ' is-invalid' : '' }}" name="address" rows="1">{{ old('address', $outlet->address) }}</textarea>
